...
export class ZardButtonComponent implements OnDestroy {
  ...
  /**
   * True when this needs synthetic button semantics - role, tabindex and a disabled attribute -
   * because the host is neither a button nor a link and nothing above it supplies them.
   *
   * LOCAL FIX to the vendored component. Upstream only inspected the PARENT element, which is
   * right for the wrapper form `<z-button>` but wrong for the attribute form `a[z-button]` that
   * this component's own selector advertises: an anchor got role="button" and tabindex="0", so
   * a control that navigates announced itself to screen readers as a button, and
   * getByRole('link') stopped matching it. Checking the host's own tag first fixes both.
   */
  protected readonly isNotInsideOfButtonOrLink = computed(() => {
    // Evaluated once; assumes neither the host nor its parent changes after mount.
    const zardButtonElement = this.elementRef.nativeElement;

    if (isButtonOrLink(zardButtonElement)) {
      return false;
    }

    return zardButtonElement.parentElement
      ? !isButtonOrLink(zardButtonElement.parentElement)
      : true;
  });
}

function isButtonOrLink(element: HTMLElement): boolean {
