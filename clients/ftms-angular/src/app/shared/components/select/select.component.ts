...
@Component({
  ...
  template: `
    <button
      ...
      [attr.aria-labelledby]="zLabelledBy() || null"
      [attr.aria-label]="zLabelledBy() ? null : triggerAriaLabel()"
    ...
    <ng-template #dropdownTemplate>
      <div
        ...
        (keydown.{arrowdown,arrowup,enter,space,escape,home,end,pageup,pagedown}.prevent)="
...
export class ZardSelectComponent implements ControlValueAccessor, OnDestroy {
  ...
  // FTMS addition. Upstream names the trigger with aria-label set to the SELECTED VALUE, so a
  // screen reader announces "Active" and never "Status" - the field's purpose is lost, which the
  // native <select> and its <label> conveyed for free. Pointing this at a visible label's id
  // restores it: the combobox is named by the label, and its content is announced as the value.
  readonly zLabelledBy = input<string>('');
  ...
  // FTMS: `element` renamed to `_element` for tsconfig's noUnusedParameters. Upstream keeps the
  // parameter for call-site symmetry; the underscore preserves that without failing our build.
  ...
  private getItemAlignedOffset(): { bottom: number; top: number } | null {
    ...
    const itemHeight =
      selectedItem.offsetHeight || selectedItem.getBoundingClientRect().height || triggerHeight;
  ...
  private navigateItems(direction: number, items: HTMLElement[]) {
    ...
    let nextIndex =
      currentIndex === -1 ? (direction > 0 ? 0 : items.length - 1) : currentIndex + direction;
