 ...
 * deliberate future restore feature. So the honest wording callers should use is "archive", not
 * "delete": the record is never destroyed, but the user genuinely cannot undo this from the UI
 * today, and a dialog that pretends otherwise would be lying in both directions.
 *
...
@Component({
  ...
  template: `
    ...
    <div class="dialog__actions">
      <button type="button" class="dialog__cancel" [disabled]="busy()" (click)="cancel()">
        Cancel
      </button>
      <button type="button" class="dialog__confirm" [disabled]="busy()" (click)="confirm()">
